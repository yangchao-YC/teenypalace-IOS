...
@interface MyMoneyViewController ()<UPPayPluginDelegate>
{
...
-(void)dateUrl:(NSString *)url Key:(int)key
{
    ...
    [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
       // NSLog(@"JSON: %@", responseObject);
        ...
        [SVProgressHUD showInfoWithStatus:errorString maskType:SVProgressHUDMaskTypeClear];//异常提示
        NSLog(@"Error: %@", error);
    }];
...
-(void)dateHandle:(int)key
{
    ...
    if (key == 0) {
        if (self.articles.count != 0) {
            ...
            [self.tableView reloadData];
            [SVProgressHUD showInfoWithStatus:@"没有订单" maskType:SVProgressHUDMaskTypeBlack];
        }
    ...
    else if (key == 1)
    {
        if ([[self.dic objectForKey:@"status"]intValue] == 0) {
            [SVProgressHUD showSuccessWithStatus:@"删除成功,正在刷新订单" maskType:SVProgressHUDMaskTypeBlack];
            
            [self selectMoney];
        }
        else
        {
            [SVProgressHUD showInfoWithStatus:[self.dic objectForKey:@"message"] maskType:SVProgressHUDMaskTypeBlack];
        }
    }
    else
    {
...
-(void)delete
{
    ...
    {
        [SVProgressHUD showInfoWithStatus:@"请选择要删除的订单" maskType:SVProgressHUDMaskTypeBlack];
    }
}
...
-(void)check
{
    ...
    {
        [SVProgressHUD showInfoWithStatus:@"请选择要支付的订单" maskType:SVProgressHUDMaskTypeBlack];
    }
}
    ...
    [requestManager POST:DATE_SEARCH_MONEY_PAY parameters:
     @{@"field_unionpay_merchantorderid":merchantorderid,
       ...
       @"field_unionpay_paymethod":@"unionpay_mobileapp_new"
           ...
           NSLog(@"Error: %@", error);
           NSString *errorString = [NSString stringWithFormat:@"%@",error];
           [SVProgressHUD showInfoWithStatus:errorString maskType:SVProgressHUDMaskTypeBlack];//异常提示
       }];
}
